...
def main():
    ...
    pkt = Ether(src=get_if_hwaddr(iface), dst="ff:ff:ff:ff:ff:ff") / IP(
        dst=addr, options = IPOption_MRI(count=0, pathid=0,
            swtraces=[])) / UDP(
 ...
 #           dport=4321, sport=1234) / sys.argv[2]
    ...
    try:
      for i in range(int(sys.argv[3])):
        pkt2 = Ether(src=get_if_hwaddr(iface), dst="ff:ff:ff:ff:ff:ff") / IP(
            dst=addr, options = IPOption_MRI(count=0, pathid=1, pcount=0,
                swtraces=[] )) / UDP(
                dport=4321, sport=1234) / sys.argv[2]
        pkt2.show2()
