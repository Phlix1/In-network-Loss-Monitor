...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, **opts):
        ...
        for link in host_links:
            ...
            host_ip = "10.0.%d.%d" % (205, host_num)
            host_mac = '00:00:00:00:%02x:%02x' % (205, host_num)
            ...
            #             delay=link['latency'], bw=link['bandwidth'], loss=link['lossrate'],
            #             addr1=host_mac, addr2=host_mac)
            self.addLink(host_name, host_sw, loss=link['lossrate'],
                         addr1=host_mac, addr2=host_mac)
        ...
        for link in switch_links:
            ...
            #            delay=link['latency'], bw=link['bandwidth'], loss=link['lossrate'])
            self.addLink(link['node1'], link['node2'],loss=link['lossrate'])
